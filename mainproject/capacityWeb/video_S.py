...
def RT_Image(object, client, D_addr, vid_name):
    ...
    while (1):
        ...
        if (data != 0):
            # print("ok")
            cv2.putText(object.img, 'number of people  ' + str(data.decode()), (70, 70), cv2.FONT_HERSHEY_COMPLEX, 1,
                        (0, 0, 255), 3)
        cv2.namedWindow("")
        cv2.imshow("", object.img)  # 读取视频每一帧
        # if(cv2.waitKey(10) == 27 ):
        #      exit("已退出")
        # count_info = struct.unpack('h', client.recv(2))
        # print(count_info[0])
        if (i % 20 == 0):
            print(i)
            print("ok")
            object.img = cv2.resize(object.img, (640, 480))  # 按要求调整图像大小(resolution必须为元组)
            ...
            try:
                client.send(struct.pack("lhh", len(object.img_data), object.resolution[0],
                                        object.resolution[1]) + object.img_data)  # 按照相应的格式进行打包发送图片 t图传指令
            except:
                camera.release()  # 释放资源
                return
            data = conn.recv(1024)
    ...
    # print("当前人数：",data.decode())
    # cv2.putText(imgcv, mess, (left, top - 12), 0, 1e-3 * h, colors[max_indx], thick // 3)

    # print("当前人数：", data.decode())

    # if not data:
    # print("Do not has data")


def connect2server(vid_name):
    camera = Carame_Accept_Object()  # 执行这个类
    print("等待连接...")
    while (1):
        client, D_addr = camera.server.accept()  # 服务器接受到来自客户端的连接，收到客户端的信息为

        clientThread = threading.Thread(None, target=RT_Image, args=(
            camera, client, D_addr, vid_name,))  # target: 是通过run()方法调用的可调用对象,args：元组参数，为target所调用的

        clientThread.start()


if __name__ == '__main__':
    connect2server('video_02.mp4')
